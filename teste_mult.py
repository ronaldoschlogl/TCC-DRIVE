# Script adaptado para utilizar UNet, EMA e SpaceSampling conforme o modelo original, com comparações visuais
...
etapas_amostragem = [125, 250, 500, 750, 1000]  # Etapas de amostragem para comparação
num_amostras = 1000
image_size = 64
timesteps = 1000
noise_type = "cos"
...
for ns in etapas_amostragem:
    ...
    for i in range(num_amostras):
        ...
        #Image.fromarray(ruido_np).save(os.path.join(pasta_resultados, f"{nome_base}_ruido.png"))
        ...
        metricas_total.append((ns, i, mse_val, ssim_val))

        # Adiciona à comparação
        resultados_comparativos.setdefault(i, {})[ns] = frame_np

        # Salva cada ssim e mse em CSV
        with open(os.path.join(pasta_resultados, "metricas.csv"), "a") as f:
            if not os.path.exists(os.path.join(pasta_resultados, "metricas.csv")):
                f.write("Etapas,Amostra,MSE,SSIM\n")
            f.write(f"{ns},{i},{mse_val},{ssim_val}\n")
...
# DataFrame e CSV
df = pd.DataFrame(metricas_total, columns=["Etapas", "Amostra", "MSE", "SSIM"])
...
for i, geracoes in resultados_comparativos.items():
    if(i % 100 == 0):
        print(f"Gerando comparação visual para amostra {i}")
        fig, axs = plt.subplots(1, len(etapas_amostragem), figsize=(12, 4))
        for j, ns in enumerate(sorted(geracoes.keys())):
            axs[j].imshow(geracoes[ns])
            axs[j].axis("off")
            axs[j].set_title(f"Etapas: {ns}")
        plt.tight_layout()
        plt.savefig(os.path.join(pasta_resultados, f"comparacao_amostra_{i}.png"))
        plt.close()
