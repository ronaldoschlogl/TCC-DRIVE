...
class GaussianDiffusion:
    def __init__(self, noise_type, timesteps, device="cpu") -> None:
        ...
        self.alphas_cumprod_prev = F.pad(
            self.alphas_cumprod[:-1], pad=(1, 0), value=1.)
        ...
        self.posterior_log_variance = torch.log(
            self.posterior_variance.clamp(min=1e-15))
        self.posterior_mean_coe1 = torch.sqrt(
            self.alphas_cumprod_prev) * self.betas / (1 - self.alphas_cumprod)
        self.posterior_mean_coe2 = torch.sqrt(
            self.alphas) * (1 - self.alphas_cumprod_prev) / (1 - self.alphas_cumprod)
    ...
    def p_sample(self, model, x_t, t, y=None):
        ...
        model_mean, model_variance, model_log_variance = self.p_mean_variance(
            pred_noise, pred_frac, x_t, t, clip_denoised=True)
    ...
    def q_sample(self, x_0, t, eps):
        ...
        sqrt_alpha_cumprod = self.sqrt_alphas_cumprod.gather(
            -1, t).view(-1, 1, 1, 1)
        sqrt_one_minus_alpha_cumprod = self.sqrt_one_minus_alphas_cumprod.gather(
            -1, t).view(-1, 1, 1, 1)
    ...
    def q_posterior_mean_variance(self, x_t, x_0, t):
        ...
        log_variance = self.posterior_log_variance.gather(
            -1, t).view(-1, 1, 1, 1)
        posterior_mean_coe1_t = self.posterior_mean_coe1.gather(
            -1, t).view(-1, 1, 1, 1)
        posterior_mean_coe2_t = self.posterior_mean_coe2.gather(
            -1, t).view(-1, 1, 1, 1)
    ...
    def p_mean_variance(self, pred_noise, pred_frac, x_t, t, clip_denoised=False):
        ...
        min_log_var = self.posterior_log_variance.gather(
            -1, t).view(-1, 1, 1, 1)
        frac = (pred_frac + 1) / 2
    ...
    def p_losses(self, model, x_0, t, y=None):
        ...
        true_mean, true_variance, true_log_variance = self.q_posterior_mean_variance(
            x_t, x_0, t)
        model_mean, model_variance, model_log_variance = self.p_mean_variance(
            pred_noise, pred_frac, x_t, t)
        # L_t-1 = KL(q(x_{t-1}|x_t,x_0) || p(x_{t-1}|x_t))
        kl = self.gaussian_kl_norm(
            true_mean, true_log_variance, model_mean.detach(), model_log_variance)
        kl = kl.mean(dim=[1, 2, 3]) / np.log(2)
        # L_0 = -log(p{x_0 | x_1})
        decoder_nll = -self.discretized_gaussian_log_likelihood(
            x_0, means=model_mean.detach(), log_scales=0.5 * model_log_variance)
    ...
    def gaussian_kl_norm(self, mean1, log_var1, mean2, log_var2):
        kl = 0.5 * (-1.0 + log_var2 - log_var1 + torch.exp(log_var1 -
                    log_var2) + ((mean1 - mean2) ** 2) * torch.exp(-log_var2))
        return kl
    ...
    def cos_beta_schedule(self, timesteps, s=0.001):
    ...
    def predict_x_0_from_noise(self, x_t, t, eps):
        sqrt_one_minus_alphas_cumprod = self.sqrt_one_minus_alphas_cumprod.gather(
            -1, t).view(-1, 1, 1, 1)
        sqrt_alphas_cumprod = self.sqrt_alphas_cumprod.gather(
            -1, t).view(-1, 1, 1, 1)
...
class SpaceSampling(GaussianDiffusion):
    def __init__(self, noise_type, timesteps, num_sample, device="cpu") -> None:
        ...
        self.S = torch.linspace(0, self.timesteps - 1,
                                num_sample, dtype=torch.int64).to(device)
        alphas_cumprod_St = self.alphas_cumprod.gather(-1, self.S)
        alphas_cumprod_St_prev = F.pad(
            alphas_cumprod_St[:-1], pad=(1, 0), value=1.)
    ...
    def p_fast_sample_loop(self, model, y=None, image_size=64):
        x_t = torch.randn(1, 3, image_size, image_size).to(self.device)
        res = []
