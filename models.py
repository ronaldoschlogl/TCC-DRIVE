...
class MultiHeadAttention(nn.Module):
    def __init__(self, ch_in, num_heads=4, num_head_channels=64, num_groups=32) -> None:
        ...
        for i in range(num_heads):
            q = nn.Conv2d(ch_in, num_head_channels,
                          kernel_size=1, stride=1, padding=0)
            k = nn.Conv2d(ch_in, num_head_channels,
                          kernel_size=1, stride=1, padding=0)
            v = nn.Conv2d(ch_in, num_head_channels,
                          kernel_size=1, stride=1, padding=0)
            self.heads.append(nn.ModuleList([q, k, v]))
        self.proj = nn.Conv2d(num_head_channels * num_heads,
                              ch_in, kernel_size=1, stride=1, padding=0)
    ...
    def attention(self, q, k, v):
        ...
        attn_score = torch.bmm(
            q.view(b, c, h*w).permute(0, 2, 1), k.view(b, c, h*w)) / math.sqrt(c)
        attn_weights = torch.softmax(attn_score, dim=-1)
        out = torch.bmm(attn_weights, v.view(
            b, c, h*w).permute(0, 2, 1)).permute(0, 2, 1).view(b, c, h, w)
        return out


class UNet(nn.Module):
    def __init__(self, image_size=32, in_channel=3, out_channel=3, num_head_channel=64, num_class=None, model_channel=128, channel_mult=[1, 2, 4, 8], attention_resolutions=[16, 8], num_res_block=3, num_heads=4, dropout=0.3, num_groups=32, device="cuda") -> None:
        super().__init__()
        self.timestep_embedding = nn.Sequential(TimestepEmbedding(model_channel, device),
                                                nn.Linear(
                                                    model_channel, model_channel*4),
                                                nn.SiLU(),
        ...
        self.input_block = nn.ModuleList([TimestepEmbedSequential(
            nn.Conv2d(in_channel, model_channel, kernel_size=3, stride=1, padding=1))])
        ch = model_channel
        for idx, mult in enumerate(channel_mult):
            for j in range(num_res_block):
                layers = [ResBlock(
                    ch, model_channel * mult, ch_emb=model_channel*4, dropout=dropout, num_groups=num_groups)]
                ch = mult * model_channel
                resolution = image_size//(2**idx)
                if resolution in attention_resolutions:
                    layers.append(MultiHeadAttention(
                        ch, num_heads=num_heads, num_head_channels=num_head_channel, num_groups=num_groups))
                self.input_block.append(TimestepEmbedSequential(*layers))
                input_block_chans.append(ch)
            if idx < len(channel_mult) - 1:
                self.input_block.append(TimestepEmbedSequential(ResBlock(
                    ch, ch, ch_emb=model_channel*4, dropout=dropout, num_groups=num_groups, is_down=True)))
                input_block_chans.append(ch)

        self.middle_block = TimestepEmbedSequential(
            ResBlock(ch, ch, ch_emb=model_channel*4,
                     dropout=dropout, num_groups=num_groups),
            MultiHeadAttention(ch, num_head_channels=num_head_channel,
        ...
        for idx, mult in list(enumerate(channel_mult))[::-1]:
            ...
            for i in range(num_res_block + 1):
                layers = []
                layers.append(ResBlock(ch + input_block_chans.pop(), model_channel *
                              mult, ch_emb=model_channel*4, dropout=dropout, num_groups=num_groups))
                ch = model_channel * mult
                if resolution * 2 ** (c-1) in attention_resolutions:
                    layers.append(MultiHeadAttention(
                        ch, num_head_channels=num_head_channel, num_heads=num_heads, num_groups=num_groups))
                if idx > 0 and i == num_res_block:
                    layers.append(ResBlock(ch, ch, ch_emb=model_channel*4,
                                  dropout=dropout, num_groups=num_groups, is_up=True))
